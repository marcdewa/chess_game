...
class QueenTest {
	...
	void correctMovementTest1() throws Exception {
		...
		assertTrue(game.board[8][4].getPiece().canMove(mc),"Queen can go straight and diagonally");
	}
	...
	void falseMovementTest1() throws Exception {
		...
		assertFalse(game.board[8][4].getPiece().canMove(mc),"Queen only can go straight and diagonally");
	}
	...
	void falseMovementTest3() throws Exception {
		...
		MoveCoordinate mc = new MoveCoordinate("D8-H8",game);
		assertFalse(game.board[8][4].getPiece().canMove(mc),"Queen can't jump over piece");
	}
